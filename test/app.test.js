...
describe('Route GET /object/:key', () => {
  ...
  test('GET request: one query parameter beside timestamp should return error', async() => {
    const response = await request(app).get(`/object/key?timeStamp=sadasd`);
    expect(response.status).toBe(404);
    expect(response.body.error).toBe('timestamp should be the only query parameter!');
  });
  test('GET request: more than one query parameters should return error', async() => {
    const response = await request(app).get("/object/key?timestamp='1212'&limit=0");
    expect(response.status).toBe(404);
    expect(response.body.error).toBe('Query parameter should not more than 1!');
  });
});
